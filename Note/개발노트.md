...
2. Amazon IAM(Identity and Access Management) 
  ...
  - AWS IAM 카테고리 -> Users 항목 -> Add user 으로 들어가 세부 내역 작성. 정책 설정
...
- python 설치 후 경로를 잡고 git bash나 cmd 창에서 pip install awscli 명령어 수행.
...
- https://aws.amazon.com/ko/sdk-for-node-js/ 라이브러리 및 설명서 참고
...
- > 명령어 실행 후 나온 텍스트를 저장해둔다.
  ...
  > stack: mlog-app-api-prod
  > resources: 43
  > api keys:
  >   None
  > endpoints:
  >   POST - https://p3auo1ehhg.execute-api.ap-northeast-2.amazonaws.com/prod/notes
  >   GET - https://p3auo1ehhg.execute-api.ap-northeast-2.amazonaws.com/prod/notes/{id}
  >   GET - https://p3auo1ehhg.execute-api.ap-northeast-2.amazonaws.com/prod/notes
  >   PUT - https://p3auo1ehhg.execute-api.ap-northeast-2.amazonaws.com/prod/notes/{id}
  >   DELETE - https://p3auo1ehhg.execute-api.ap-northeast-2.amazonaws.com/prod/notes/{id}
  >   POST - https://p3auo1ehhg.execute-api.ap-northeast-2.amazonaws.com/prod/billing
  > functions:
  >   create: mlog-app-api-prod-create
  >   get: mlog-app-api-prod-get
  >   list: mlog-app-api-prod-list
  >   update: mlog-app-api-prod-update
  >   delete: mlog-app-api-prod-delete
  >   billing: mlog-app-api-prod-billing
  > layers:
  >   None
...
- test하기위한 명령어 실행
  ...
  - > npx aws-api-gateway-cli-test --username='popuyaha@naver.com' --password='ansrudghks1!' --user-pool-id='ap-northeast-2_AopDentQv' --app-client-id='50rek42h3dnk3fk6roed2nol4p' --cognito-region='ap-northeast-2' --identity-pool-id='ap-northeast-2:59cd5099-9876-42fd-98ca-2d74936ae374' --invoke-url='https://p3auo1ehhg.execute-api.ap-northeast-2.amazonaws.com/prod' --api-gateway-region='ap-northeast-2' --path-template='notes' --method='POST' --body='{"content":"hello world","attahment":"hello.jpg"}'

19. 프론트엔드 세팅하기 
    - create-vue-app 으로 초기 세팅을 해준다.
    - npm run dev로 기동
20. favicon 설정
    - https://realfavicongenerator.net/ 에서 원하는 이미지 선택 후 생성
    - favicon.ico파일 변경
21. vue를 위한 bootstrapVue, bootstrap 설치
    - npm install bootstrap-vue bootstrap vue-router 실행
    - index.js에 bootstrap 세팅 후 화면 확인
    - `vue-router`와 같은 Vue.js 공식 플러그인이 제공하는 일부 플러그인은`Vue`가 전역 변수로 사용 가능한 경우 자동으로`Vue.use()`를 호출하므로 use()를 적을 필요가 없다.
22. 